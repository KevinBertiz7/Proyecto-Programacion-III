  ...
  
{
    public class RepositorioClientes : ConexionBD, DatosDAO.ICRUD<Cliente>
    {
        ...
        public string Insertar(Cliente obj)
        {
            try
            {
                string _sql = string.Format("INSERT INTO [dbo].[Clientes] ([IdCliente], [Nombres], [TipoCliente]) VALUES('{0}', '{1}', '{2}')", obj.IDC, obj.Nombre, obj.TipoCliente);
                var cmd = new SqlCommand(_sql, conexion);
                AbrirConnexion();
                int filas = cmd.ExecuteNonQuery();
                CerrarConnexion();
            ...
            {
                return "Persona con identificacion : " + obj.IDC + " No agregado, Posible persona existente";
            }
        }
